...
public class UserRestEndpoint {
	...
	public HttpEntity<UserResource> createUser(@Valid @RequestBody final UserResource userRes) {
		...
		return new ResponseEntity<>(this.userResourceAsm.toResource(savedUser), HttpStatus.CREATED);
	}

	@RequestMapping(method = RequestMethod.GET)
	public HttpEntity<List<UserResource>> showAllUsers() {
		final List<User> userList = this.userService.findAll();
		return new ResponseEntity<>(this.userResourceAsm.toResources(userList), HttpStatus.OK);
	}

	@RequestMapping(value = "/{userId}", method = RequestMethod.GET)
	public HttpEntity<UserResource> getUser(@PathVariable final long userId) {
		final User user = this.userService.findOne(userId);
		return new ResponseEntity<>(this.userResourceAsm.toResource(user), HttpStatus.OK);
	}
	...
	public HttpEntity<UserResource> updateUser(@Valid @RequestBody final UserResource userRes, @PathVariable final long userId) {
		...
		return new ResponseEntity<>(this.userResourceAsm.toResource(updated), HttpStatus.OK);
	}
	...
	public HttpEntity<List<IdeaResource>> getUserIdeas(@PathVariable final long userId) {
		...
		return new ResponseEntity<>(this.ideaResourceAsm.toResources(userIdeas), HttpStatus.OK);
	}
...
//		return new ResponseEntity<>(thi.toResource(savedIdea), HttpStatus.CREATED);
//	}
	...
	public HttpEntity<List<CommentResource>> getUserComments(@PathVariable final long userId) {
		...
		return new ResponseEntity<>(this.commentResourceAsm.toResources(comments), HttpStatus.OK);
	}
