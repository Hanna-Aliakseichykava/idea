...
public class TagRestEndpointTest {
	...
	public void setUp() throws Exception {
		...
		tag = TestTagBuilder.aTag().build();
		idea = TestIdeaBuilder.anIdea().build();
		idea.addTag(tag);
		tag.addIdea(idea);
		user.addIdea(idea);
		idea.setAuthor(user);
		given(this.tagServiceMock.getIdeasCountForTag(anyString())).willReturn(tag.getIdeasWithTag().size());
	}

	@Test
	public void shouldReturnAllFoundTags() throws Exception {
		given(this.tagServiceMock.findAll()).willReturn(Lists.newArrayList(tag));
		...
		this.mockMvc.perform(get("/api/v1/tags")
				...
				.andExpect(jsonPath("$[0].ideasCount").value(tag.getIdeasWithTag().size()))
				.andExpect(jsonPath("$[0].links", hasSize(2)))
				.andExpect(jsonPath("$[0].links[0].rel").value(Link.REL_SELF))
				.andExpect(jsonPath("$[0].links[0].href").value(containsString("/api/v1/tags/" + tag.getId())))
				.andExpect(jsonPath("$[0].links[1].rel").value(TagResourceAsm.IDEAS_REL))
				.andExpect(jsonPath("$[0].links[1].href").value(containsString("/api/v1/tags/" + tag.getId() + "/ideas")));

		verify(this.tagServiceMock, times(1)).findAll();
		verify(this.tagServiceMock, times(1)).getIdeasCountForTag(tag.getName());
		verifyNoMoreInteractions(this.tagServiceMock);
	}

	@Test
	public void shouldReturnTagByIdWithHttpCode200() throws Exception {
		given(this.tagServiceMock.findOne(tag.getId())).willReturn(tag);

		this.mockMvc.perform(get("/api/v1/tags/{tagId}", tag.getId())
				.accept(APPLICATION_JSON_UTF8)).andDo(print())
				.andExpect(status().isOk())
				.andExpect(content().contentType(APPLICATION_JSON_UTF8))
				.andExpect(jsonPath("$.name").value(tag.getName()))
				.andExpect(jsonPath("$.ideasCount").value(tag.getIdeasWithTag().size()))
				.andExpect(jsonPath("$.links", hasSize(2)))
				.andExpect(jsonPath("$.links[0].rel").value(Link.REL_SELF))
				.andExpect(jsonPath("$.links[0].href").value(containsString("/api/v1/tags/" + tag.getId())))
				.andExpect(jsonPath("$.links[1].rel").value(TagResourceAsm.IDEAS_REL))
				.andExpect(jsonPath("$.links[1].href").value(containsString("/api/v1/tags/" + tag.getId() + "/ideas")));

		verify(this.tagServiceMock, times(1)).findOne(tag.getId());
		verify(this.tagServiceMock, times(1)).getIdeasCountForTag(tag.getName());
		verifyNoMoreInteractions(this.tagServiceMock);
	}
	...
	public void shouldReturnErrorWithHttpStatus404WhenTagNotFound() throws Exception {
		given(this.tagServiceMock.findOne(TestTagBuilder.DEFAULT_ID)).willThrow(new TagDoesNotExistException());
	...
	public void shouldReturnAllFoundIdeasForGivenTag() throws Exception {
		given(this.ideaServiceMock.findIdeasByTagId(tag.getId())).willReturn(Lists.newArrayList(idea));
		...
		this.mockMvc.perform(get("/api/v1/tags/{tagId}/ideas", tag.getId())
				...
				.andExpect(jsonPath("$[0].tags[0].ideasCount").value(tag.getIdeasWithTag().size()))
				...
				.andExpect(jsonPath("$[0].links[0].href").value(containsString("/api/v1/users/" + idea.getAuthor().getId())));
